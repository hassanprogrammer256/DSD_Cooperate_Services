...
import { ApiError, STATIC_SITE_MESSAGE, apiClient, setAccessToken } from "@/lib/api/client";
import {
  authKeys,
  useLoginMutation,
  useLogoutMutation,
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  async function loginWithGoogle() {
    // Static build: Google sign-in needs the backend to verify the Firebase token,
    // so fail fast instead of opening a popup that can't complete.
    throw new ApiError(503, STATIC_SITE_MESSAGE);
  }
